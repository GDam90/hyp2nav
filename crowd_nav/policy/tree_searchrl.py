...
class TreeSearchRL(Policy):
    ...
    def configure(self, config, device, embedding_dimension=128, hyperbolic=False, curvature=-1.0, radius=False):

        self.hyperbolic = hyperbolic #True if config.hyperbolic else False
        ...
        if self.exploration_alg == "dropout":
            ...
            if self.linear_state_predictor:
                ...
                if self.share_graph_model:
                    ...
                    graph_model1 = GAT_RL(config, self.robot_state_dim, self.human_state_dim)
                    self.value_estimator = DQNNetwork(config, graph_model1, self.embedding_dim, self.hyperbolic, self.curvature)
                    graph_model2 = GAT_RL(config, self.robot_state_dim, self.human_state_dim)
                    self.state_predictor = StatePredictor(config, graph_model2, self.time_step)
                    self.model = [graph_model1, graph_model2, self.value_estimator.value_network,
                                self.state_predictor.human_motion_predictor]
    ...
    def predict(self, state, args=None):
        """
        ...
        if self.exploration_alg == "epsilon_greedy":
            ...
            if self.phase == 'train' and probability < self.epsilon and self.use_noisy_net is False:
                ...
                self.exploration_alg == "random_encoder" or self.exploration_alg == "noisy_net" or self.exploration_alg == "hypercuriosity" or
                self.exploration_alg == "curiosity_hyperdistance"):
            # if self.phase == 'test_viz':
            #     max_value, max_action_index, max_traj, radius = self.V_planning(state_tensor, self.planning_depth, self.planning_width)
            # else:
            #     max_value, max_action_index, max_traj = self.V_planning(state_tensor, self.planning_depth, self.planning_width)
            if args:
                max_value, max_action_index, max_traj, returns, max_action_indexes, radius = self.V_planning(state_tensor, self.planning_depth, self.planning_width, visualize=args.visualize)
                not_best_actions = [self.action_space[max_action_indexes[i]] for i in range(max_action_indexes.shape[0])]
            else:
                max_value, max_action_index, max_traj = self.V_planning(state_tensor, self.planning_depth, self.planning_width)
            # max_value, max_action_index, max_traj = self.V_planning(state_tensor, self.planning_depth, self.planning_width)
            if max_value[0] > origin_max_value:
        ...
        if args:
            return max_action, int(max_action_index[0]), returns, not_best_actions, max_action_indexes, radius
        return max_action, int(max_action_index[0])
        # if self.phase == 'test_viz':
        #     return max_action, int(max_action_index[0]), radius
        # else:
        #     return max_action, int(max_action_index[0])
        # # return max_action, int(max_action_index[0])

    def V_planning(self, state, depth, width, visualize=False):
        ...
        if state[1] is None:
            ...
            next_values, next_action_indexes, next_trajs = self.V_planning(next_state_batch, depth - 1, cur_width, visualize)
            ...
            if depth == 0:
                if visualize:
                    q_value, radius = self.value_estimator(state, radius=True)
                else:
                    q_value = self.value_estimator(state)
                if self.exploration_alg == "boltzmann":
                    q_dist = torch.softmax(q_value/self.epsilon,-1,float)
                ...
                if visualize: #self.phase == "test_viz": #! aggiungere che la fase debba essere viz
                    q_value, radius = self.value_estimator(state, radius=True)
                else:
                    q_value = self.value_estimator(state)
                # q_value = self.value_estimator(state, report_radius) # here is were it calculates the q_value, so we can log from here
                if self.exploration_alg == "boltzmann":
                    q_dist = torch.softmax(q_value/self.epsilon,-1,float)
                    max_action_indexes = torch.stack([torch.multinomial(q_dist_aux, width) for q_dist_aux in q_dist])
                    max_action_value = torch.gather(q_value, 1, max_action_indexes)
                else:
                    max_action_value, max_action_indexes = torch.topk(q_value, width, dim=1) # width is the number of actions to be considered
            action_stay = []
            for i in range(robot_state_batch.shape[0]):
                if self.kinematics == "holonomic":
                    action_stay.append(ActionXY(0, 0))
                    ...
                    action_stay.append(ActionRot(0, 0))
            _, pre_next_state = self.state_predictor(state, action_stay) # consider only the humans state
            next_robot_state_batch = None
            next_human_state_batch = None
            ...
            for i in range(robot_state_batch.shape[0]):
                ...
                for j in range(width):
                    ...
                    reward_est[i][j], _ = self.reward_estimator.estimate_reward_on_predictor(
                        tensor_to_joint_state(cur_state), tensor_to_joint_state((next_robot_state, next_human_state)))
            next_state_batch = (next_robot_state_batch, next_human_state_batch) # ([10, 1, 9], [10, 1, 5])
            if self.planning_depth - depth >= 2 and self.planning_depth > 2:
                cur_width = 1
            else:
                cur_width = int(self.planning_width/2)
            ...
            max_returns = []
            max_actions = []
            if visualize: #self.phase=="test_viz":
                max_radius = []
            for i in range(robot_state_batch.shape[0]):
                cur_state = (robot_state_batch[i, :, :].unsqueeze(0), human_state_batch[i, :, :].unsqueeze(0))
                action_id = max_action_index[i]
                ...
                max_actions.append(action)
                if visualize: #self.phase=="test_viz":
                    max_radius.append(radius)
            max_returns = torch.tensor(max_returns)
            # if self.phase=="test_viz":
            #     return max_returns, max_actions, trajs, max_radius
            # else:
            #     return max_returns, max_actions, trajs
            if visualize:
                return max_returns, max_actions, trajs, returns.squeeze(), max_action_indexes.squeeze(), max_radius
            return max_returns, max_actions, trajs
