...
def compute_states_entropy(obs_embeds, embed_dim: int, k_nn: int):
...
class GraphModel(nn.Module):
    ...
    def rotate(self, state):
        ...
        if len(state[1].shape) == 3:
            ...
            cur_heading = (robot_state[:, :, 8].unsqueeze(1) - rot + np.pi) % (
                2 * np.pi
            ) - np.pi
            ...
            new_human_state = torch.cat(
                (human_positions, human_velocities, human_radius), dim=2
            )
            ...
            sin_rot = torch.sin(rot)
            vx = (
                robot_state[:, :, 2].unsqueeze(1) * cos_rot
                + robot_state[:, :, 3].unsqueeze(1) * sin_rot
            ).reshape((batch, 1, -1))
            vy = (
                robot_state[:, :, 3].unsqueeze(1) * cos_rot
                - robot_state[:, :, 2].unsqueeze(1) * sin_rot
            ).reshape((batch, 1, -1))
            v_pref = robot_state[:, :, 7].unsqueeze(1)
            ...
            new_robot_state = torch.cat(
                (px_r, py_r, vx, vy, radius_r, dg, rot, v_pref, theta), dim=2
            )
...
class CADRLModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.model = nn.Sequential(nn.Linear(13, 150), nn.ReLU())
...
class LSTMModel(nn.Module):
    ...
    def forward(self, state):
        ...
        packed_mlp1_output = torch.nn.utils.rnn.pack_padded_sequence(
            mlp1_output, [10], batch_first=True
        )
...
class SARLModel(nn.Module):
    ...
    def forward(self, state):
        ...
        global_state = (
            global_state.expand((size[0], size[1], 100)).contiguous().view(-1, 100)
        )
...
class MPRLModel(nn.Module):
    ...
    def trans_no_rotation(self, state):
        ...
        theta = robot_state[:, :, 8].unsqueeze(1)
        new_robot_state = torch.cat(
            (theta, theta, vx, vy, radius_r, dg, rot, v_pref, theta), dim=2
        )
        ...
        for i in range(human_num):
            ...
            vx1 = (human_state[:, i, 2].unsqueeze(1).unsqueeze(2)).reshape(
                (batch, 1, -1)
            )
            vy1 = (human_state[:, i, 3].unsqueeze(1).unsqueeze(2)).reshape(
                (batch, 1, -1)
            )
...
class RE3:
    ...
    def __init__(
        ...
        if config.name == "cadrl":
            self._encoder_net = nn.Sequential(
                CADRLModel(), nn.Linear(150, 256), nn.ReLU(), nn.Linear(256, 128)
            )
        elif config.name == "lstm_rl":
            self._encoder_net = nn.Sequential(
                LSTMModel(), nn.Linear(150, 256), nn.ReLU(), nn.Linear(256, 128)
            )
        elif config.name == "sarl":
            self._encoder_net = nn.Sequential(
                SARLModel(), nn.Linear(150, 256), nn.ReLU(), nn.Linear(256, 128)
            )
    ...
    def compute_intrinsic_reward_batch(self, embeddings, iteration):
        states_entropy = compute_states_entropy(embeddings, self.embeds_dim, self.k_nn)
