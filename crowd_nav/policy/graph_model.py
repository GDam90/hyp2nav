...
class GAT_RL(nn.Module):
    ...
    def compute_adjectory_matrix(self, state):
        ...
        for i in range(robot_num, robot_num+human_num):
            adj[i][0] = 0 # distances with robot are not considered
    ...
    def forward(self, state):
        ...
        :return:
        """
        robot_state, human_states = state # [1, 1, 9], [1, 5, 5]
        if human_states is None:
            ...
            # compute feature matrix X
            robot_state_embedings = self.w_r(robot_state) # [1, 1, 32]
            human_state_embedings = self.w_h(human_states) # [1, 5, 32]
            X = torch.cat([robot_state_embedings, human_state_embedings], dim=1)
            if robot_state.shape[0]==1:
                H1, self.attention_weights = self.gat0(X, adj)
            else:
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        # shape of input is batch_size, graph_size,feature_dims [1, 6, 32]
        # shape of adj is batch_size, graph_size, graph_size [1, 6, 6]
        assert len(input.shape) == 3
        assert len(adj.shape) == 3
        A = self.compute_similarity_matrix(input) # [1, 6, 6]
        e = self.leakyrelu(A)
        zero_vec = -9e15 * torch.ones_like(e) # then it will be softmaxed to 0
        attention = torch.where(adj.to(input.device) > 0, e, zero_vec)
        attention = nn.functional.softmax(attention, dim=2)
        next_H = torch.matmul(attention, input) # [1, 6, 32]
    ...
    def compute_similarity_matrix(self, X):
        ...
        pairwise_features = selected_features.reshape((-1, X.size(1) * X.size(1), X.size(2) * 2))
        A = self.w_a(pairwise_features).reshape(-1, X.size(1), X.size(1)) # [1, 6, 6]
        return A
