...
from crowd_nav.utils.hyperbolic import ExpMapProjectionLayer as ExpMapLayer
from crowd_nav.utils.hyperbolic import HypLinear as HLinear
...
class ICM():
    ...
    def __init__(self, config, robot_state_dim, human_state_dim, device, lr, scaling_factor, policy="tree_search_rl", embedding_dimension=128):
        ...
        #         nn.Linear(config.gcn.X_dim, self.embedding_dimension * 2),
        #         nn.ReLU(),
        #         nn.Linear(self.embedding_dimension * 2, self.embedding_dimension)
        #     )
        # else:
        #     raise ValueError("Curiosity not implemented for this method")
        # self._curiosity_inverse_fcnet = nn.Sequential(
        #     nn.Linear(2*self.embedding_dimension, 32),
        #     nn.ReLU(),
        #     nn.Linear(32, self.embedding_dimension),
        #     nn.ReLU(),
        #     nn.Linear(self.embedding_dimension, self.action_num)
        # )

        # self._curiosity_forward_fcnet = nn.Sequential(
        #     nn.Linear(self.embedding_dimension+self.action_num, 32*2),
        #     nn.ReLU(),
        #     nn.Linear(32*2, 32),
        #     nn.ReLU(),
        #     nn.Linear(32, self.embedding_dimension)
        ...
        self._curiosity_forward_fcnet = nn.Sequential(
            nn.Linear(self.embedding_dimension+self.action_num, 256),
            nn.ReLU(),
            nn.Linear(256, 128),
            nn.ReLU(),
            nn.Linear(128, self.embedding_dimension)
        )
        self.get_exploration_optimizer()
...
class HHICM():
    ...
    def __init__(self, config, robot_state_dim, human_state_dim, device, lr, scaling_factor, policy="tree_search_rl", embedding_dimension=128, curvature=-1.):
        self.scaling_factor = scaling_factor
        self.action_num = config.action_space.speed_samples * config.action_space.rotation_samples + 1
        self.device = device
        self.lr = lr
        ...
        if policy == "cadrl":
            ...
            self._curiosity_feature_net = nn.Sequential(
                GraphModel(config, robot_state_dim, human_state_dim), # Projection layer here
                ExpMapLayer(c=torch.tensor(self.curvature)),
                HLinear(config.gcn.X_dim, self.embedding_dimension*2, c=self.curvature),
                nn.ReLU(),
                HLinear(self.embedding_dimension*2, self.embedding_dimension, c=self.curvature)
            )
        else:
            raise ValueError("Curiosity not implemented for this method")
        ...
        self._curiosity_inverse_fcnet = nn.Sequential(
            ExpMapLayer(c=torch.tensor(self.curvature)),
            HLinear(2*self.embedding_dimension, 32, c=self.curvature),
            HLinear(32, self.embedding_dimension, c=self.curvature),
            nn.ReLU(),
            HLinear(self.embedding_dimension, self.action_num, c=self.curvature)
        )

        self._curiosity_forward_fcnet = nn.Sequential(
            ExpMapLayer(c=torch.tensor(self.curvature)),
            HLinear(self.embedding_dimension+self.action_num, 32*2, c=self.curvature),
            HLinear(32*2, 32, c=self.curvature),
            nn.ReLU(),
            HLinear(32, self.embedding_dimension, c=self.curvature)
        )
        self.get_exploration_optimizer()
    ...
    def get_exploration_optimizer(self):
        ...
        self._optimizer = RiemannianAdam(
            forward_params + inverse_params + feature_params, lr=self.lr
        )
    ...
    def compute_intrinsic_reward(self, state, next_state, action):
        ...
        # next_phi = gmath.project(gmath.expmap0(next_phi, k=torch.tensor(-1.)), k=torch.tensor(-1.))
        log_phi = gmath.project(gmath.logmap0(phi, k=torch.tensor(self.curvature)), k=torch.tensor(self.curvature))

        predicted_next_phi = self._curiosity_forward_fcnet(
            torch.cat((log_phi, F.one_hot(torch.Tensor([action]).to(torch.int64).to(phi.device), self.action_num).float()), 1)
        )
        # hyperbolic projection
        # predicted_next_phi = gmath.project(gmath.expmap0(predicted_next_phi, k=torch.tensor(-1.)), k=torch.tensor(-1.))
    ...
    def compute_intrinsic_reward_batch(self, state, next_state, action):
        ...
        predicted_next_phi = self._curiosity_forward_fcnet(
            torch.cat((log_phi, F.one_hot(action.to(torch.int64), self.action_num).float()), 1)
        )
        # hyperbolic projection
        # predicted_next_phi = gmath.project(gmath.expmap0(predicted_next_phi, k=torch.tensor(-1.)), k=torch.tensor(-1.))
    ...
    def optimize(self, state, next_state, action):
        ...
        next_phi = self._curiosity_feature_net(next_state)
        # hyperbolic projection
        # next_phi = gmath.project(gmath.expmap0(next_phi, k=torch.tensor(self.curvature)), k=torch.tensor(self.curvature))
        log_phi = gmath.project(gmath.logmap0(phi, k=torch.tensor(self.curvature)), k=torch.tensor(self.curvature))

        real_actions = F.one_hot(action.to(torch.int64), self.action_num).float()

        predicted_next_phi = self._curiosity_forward_fcnet(
            torch.cat((log_phi, real_actions), 1)
        )
        # hyperbolic projection
        # predicted_next_phi = gmath.project(gmath.expmap0(predicted_next_phi, k=torch.tensor(self.curvature)), k=torch.tensor(self.curvature))

        poincare_dist = self.compute_poincare_distance(next_phi, predicted_next_phi)
        
        #poincarè distance
        squnorm = torch.sum(predicted_next_phi**2, dim=-1)
        sqvnorm = torch.sum(next_phi ** 2, dim=-1)
        radius_pred_phi = squnorm.sqrt().detach().cpu().numpy()
        radius_phi = sqvnorm.sqrt().detach().cpu().numpy()
        ...
        return forward_loss.item(), inverse_loss.item(), radius_pred_phi, radius_phi
