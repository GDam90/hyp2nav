...
class StatePredictor(nn.Module):
    ...
    def forward(self, state, action, detach=False):
        ...
        if len(state[1].shape) == 3:
            state_embedding = self.graph_model(state) # [1, 6, 32]
            if detach:
            ...
            if action is None:
                ...
                # else:
                next_robot_state = self.compute_next_states(state[0], action) # [1, 1, 1, 1, 9] (real, application of action)
            next_human_states = self.human_motion_predictor(state_embedding)[:, 1:, :] # [1, 5, 5] (predicted)
