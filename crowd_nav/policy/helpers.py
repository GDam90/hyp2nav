...
import pdb
# hypll is not compatible with pytorch 3.7
...
class HyperDuelingDQN(nn.Module):

    def __init__(self, input_dim, output_dim, embedding_dimension=128, curvature=-1.):
        super(HyperDuelingDQN, self).__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
        self.curvature = curvature
        self.embedding_dimension = embedding_dimension
        print('embedding_dimension', self.embedding_dimension)

        self.feauture_layer = nn.Sequential(
            HLinear(self.input_dim, 32, c=self.curvature), # orig: 128
            # nn.HReLU(),
        ...
        self.value_stream = nn.Sequential(
            HLinear(self.embedding_dimension, 32, c=self.curvature),
            nn.ReLU(),
            HLinear(32, 1, c=self.curvature)
        )

        self.advantage_stream = nn.Sequential(
            HLinear(self.embedding_dimension, 32, c=self.curvature),
            nn.ReLU(),
            HLinear(32, self.output_dim, c=self.curvature)
        )
        # for p in self.advantage_stream.parameters():
        #     p.requires_grad = False

    def forward(self, state, radius=False):
        # state = pmath.project(pmath.expmap0(state, c=self.c), c=self.c)
        state = gmath.project(gmath.expmap0(state, k=torch.tensor(self.curvature)), k=torch.tensor(self.curvature))
        features = self.feauture_layer(state)
        if radius:
            # pdb.set_trace()
            hyper_radius = torch.norm(features.squeeze())
            hyper_radius = [hyper_radius.detach().cpu().numpy(), features.detach().cpu().numpy()]
        values = self.value_stream(features)
        advantages = self.advantage_stream(features)
        qvals = values + (advantages - advantages.mean())
        if radius:
            return qvals, hyper_radius
        else:
            return qvals

class DuelingDQN(nn.Module):

    def __init__(self, input_dim, output_dim, embedding_dimension=128):
        ...
        self.value_stream = nn.Sequential(
            nn.Linear(self.embedding_dimension, 128),
            nn.ReLU(),
            nn.Linear(128, 1)
        )
        ...
        # self.value_stream = nn.Sequential(
        #     nn.Linear(self.embedding_dimension, 32),
        #     nn.ReLU(),
        #     nn.Linear(32, 1)
        # )
...
class DuelingDropoutDQN(nn.Module):

    def __init__(self, input_dim, output_dim):
        super(DuelingDropoutDQN, self).__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
        self.p = 0.0
        self.feature1 = nn.Linear(self.input_dim, 128)
        self.feature2 = nn.Linear(128, 128)
        self.value1 = nn.Linear(128, 128)
        self.value2 = nn.Linear(128, 1)
        self.advantage1 = nn.Linear(128, 128)
        self.advantage2 = nn.Linear(128, self.output_dim)
        # self.feauture_layer = nn.Sequential(
        #     nn.Linear(self.input_dim, 128),
        #     # nn.LeakyReLU(negative_slope=-0.2)
        #     nn.ReLU(),
    ...
    def forward(self, state):
        features = self.feature1(state)
        features = F.relu(features)
        features = F.dropout(features, self.p, self.training)
        ...
        values = F.dropout(values, self.p, self.training)
        values = self.value2(values)

        advantages = self.advantage1(features)
        advantages = F.relu(advantages)
        advantages = F.dropout(advantages, self.p, self.training)
        advantages = self.advantage2(advantages)
