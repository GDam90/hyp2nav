...
from crowd_nav.policy.cadrl import dropout_mlp, mlp
...
class ValueEstimator(nn.Module):
    def __init__(self, config, graph_model, exploration_alg=None):
        super().__init__()
        self.graph_model = graph_model
        if exploration_alg == "dropout":
            self.value_network = dropout_mlp(config.gcn.X_dim, config.model_predictive_rl.value_network_dims, exploration_alg=exploration_alg)
        else:
            self.value_network = mlp(config.gcn.X_dim, config.model_predictive_rl.value_network_dims, exploration_alg=exploration_alg)
...
class DQNNetwork(nn.Module):
    def __init__(self, config, graph_model, embedding_dim, hyperbolic, curvature=-1.0):
        super().__init__()
        ...
        if self.hyperbolic:
            self.value_network = HyperDuelingDQN(config.gcn.X_dim, self.action_num, self.embedding_dim, self.curvature)
        else:
            self.value_network = DuelingDQN(config.gcn.X_dim, self.action_num, self.embedding_dim)
            # self.value_network = NoisyDuelingDQN(config.gcn.X_dim, self.action_num)
    def forward(self, state, radius=False): #! aggiunto radius perché voglio riportarlo
        """ Embed state into a latent space. Take the first row of the feature matrix as state representation.
        """
        assert len(state[0].shape) == 3
        # assert len(state[1].shape) == 3
        ...
        # only use the feature of robot node as state representation
        state_embedding = self.graph_model(self.rotate(state))[:, 0, :] # [1, 32]
        if radius:
            value, radius = self.value_network(state_embedding, radius) # [1, 81] values for each action #! qui va aggiunto radius credo
        else:
            value = self.value_network(state_embedding)
        if radius: return value, radius
        return value
    ...
    def rotate(self, state):
        ...
        if len(state[1].shape) == 3:
            ...
            human_state = state[1]
            human_num = state[1].shape[1]
            dx = robot_state[:, :, 5] - robot_state[:, :, 0] # px - gx
            dy = robot_state[:, :, 6] - robot_state[:, :, 1] # py - gy
            dx = dx.unsqueeze(1)
            dy = dy.unsqueeze(1)
            dg = torch.norm(torch.cat([dx, dy], dim=2), 2, dim=2, keepdim=True)
            rot = torch.atan2(dy, dx)
            ...
            sin_rot = torch.sin(rot)
            transform_matrix = torch.cat((cos_rot, -sin_rot, sin_rot, cos_rot), dim=1).reshape(batch, 2, 2)
            robot_velocities = torch.bmm(robot_state[:, :, 2:4], transform_matrix) # vx and vy rotated
            radius_r = robot_state[:, :, 4].unsqueeze(1)
            v_pref = robot_state[:, :, 7].unsqueeze(1)
            target_heading = torch.zeros_like(radius_r)
            pos_r = torch.zeros_like(robot_velocities)
            cur_heading = (robot_state[:, :, 8].unsqueeze(1) - rot + np.pi) % (2 * np.pi) - np.pi
            new_robot_state = torch.cat((pos_r, robot_velocities, radius_r, dg, target_heading, v_pref, cur_heading), dim=2)
            human_positions = human_state[:, :, 0:2] - robot_state[:, :, 0:2] # old robot_state
            human_positions = torch.bmm(human_positions, transform_matrix)
            human_velocities = human_state[:, :, 2:4]
            human_velocities = torch.bmm(human_velocities, transform_matrix)
            human_radius = human_state[:, :, 4].unsqueeze(2) + 0.3
...
class DQNDropoutNetwork(nn.Module):
    def __init__(self, config, graph_model):
        ...
        # self.value_network = mlp(config.gcn.X_dim, config.model_predictive_rl.value_network_dims)
        # self.value_network = DQN(config.gcn.X_dim, 25)
        self.value_network = DuelingDropoutDQN(config.gcn.X_dim, self.action_num)
        # self.value_network = NoisyDuelingDQN(config.gcn.X_dim, self.action_num)
    
    def set_dropout(self, p):
        self.value_network.set_dropout(p)
