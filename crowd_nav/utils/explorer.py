...
class Explorer(object):
    ...
    def run_k_episodes(self, k, phase, update_memory=False, imitation_learning=False, episode=None, epoch=None,
        ...
        if phase in ['test', 'val'] or imitation_learning:
            ...
            pbar = None
        if self.robot.policy.name in ['model_predictive_rl', 'tree_search_rl']: #? name is TreeSearchRL, maybe should be TreeSearchRL instead of tree_search_rl
        ...
        for i in range(k):
            ob = self.env.reset(phase) # list with n_humans ObservableState objects (1 per person)
            done = False
            states = []
            actions = []
            rewards = []
            ...
            if isinstance(info, ReachGoal):
                ...
                sim_time.append(self.env.global_time)
            elif isinstance(info, Collision):
                collision += 1
                collision_cases.append(i)
                collision_times.append(self.env.global_time)
                if phase in ['test']:
                    print(f'collision happen {self.env.global_time} at simulation {i}')
                sim_time.append(self.env.global_time)
            elif isinstance(info, Timeout):
                timeout += 1
                timeout_cases.append(i)
                if phase in ['test']:
                    print('timeout happen', self.env.global_time)
                    rewards[-1] = rewards[-1]-0.25
                timeout_times.append(self.env.time_limit)
                sim_time.append(self.env.time_limit)
            else:
                raise ValueError('Invalid end signal from environment')
            
            if phase == 'test':
                print(f'episode {i}, CT: {sim_time[-1]}, SR: {success / (i + 1):.3f}, CR: {collision / (i + 1):.3f}, TR: {timeout / (i + 1):.3f}, NT: {average(sim_time):.3f}')
    ...
    def log(self, tag_prefix, global_step):
        ...
        wandb.log({tag_prefix + '/success_rate': sr, tag_prefix + '/collision_rate': cr, tag_prefix + '/time': time,
                   tag_prefix + '/reward': reward, tag_prefix + '/avg_return': avg_return}, step=global_step)
