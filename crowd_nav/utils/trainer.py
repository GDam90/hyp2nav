...
# from hypll.optim import RiemannianAdam, RiemannianSGD #No compatibility
...
class TSRLTrainer(object):
    def __init__(self, value_estimator, state_predictor, memory, device, policy, writer, batch_size, optimizer_str, human_num,
        ...
        self.hyperbolic = value_estimator.hyperbolic
        ...
        self.intrinsic_reward_alg = intrinsic_reward #ICM
    ...
    def set_learning_rate(self, learning_rate):
        if not self.hyperbolic:
            ...
            if self.optimizer_str == 'Adam':
                self.v_optimizer = RiemannianAdam(self.value_estimator.parameters(), lr=learning_rate)
                if self.state_predictor.trainable:
                    # currently, only the value estimator is hyperbolic
                    # self.s_optimizer = RiemannianAdam(self.state_predictor.parameters(), lr=learning_rate)
                    self.s_optimizer = optim.Adam(self.state_predictor.parameters(), lr=learning_rate)
            elif self.optimizer_str == 'SGD':
                self.v_optimizer = RiemannianSGD(self.value_estimator.parameters(), lr=learning_rate, momentum=0.9)
                if self.state_predictor.trainable:
                    # currently, only the value estimator is hyperbolic
                    # self.s_optimizer = RiemannianSGD(self.state_predictor.parameters(), lr=learning_rate, momentum=0.9)
                    self.s_optimizer = optim.SGD(self.state_predictor.parameters(), lr=learning_rate,momentum=0.9)
            else:
                raise NotImplementedError
    ...
    def optimize_epoch(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            wandb.log({'IL/epoch_v_loss': epoch_v_loss / len(self.memory), 'IL/epoch_s_loss': epoch_s_loss / len(self.memory)})
    ...
    def optimize_batch(self, num_batches, episode):
        ...
        wandb.log({'RL/average_v_loss': average_v_loss, 'RL/average_s_loss': average_s_loss}, step=episode)
